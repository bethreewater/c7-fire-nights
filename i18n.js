...
(function () {
  ...
  DICT.en = [
    ...
    ['#faq .qa:nth-of-type(6) .q', '<span class="num">07.5</span>Allergies or dietary needs?'],
    ['#faq .qa:nth-of-type(6) .a', 'No duck, shellfish, egg or dairy, vegetarian, no spice — all have alternatives. Tell us when you book; we can’t adjust on the night if it wasn’t noted.'],
    ['#faq .qa:nth-of-type(7) .q', '<span class="num">07.6</span>Do we have to leave right after?'],
    ['#faq .qa:nth-of-type(7) .a', 'No rush. After the hundred and ten minutes, the bar returns to normal service — stay for another glass.'],
    /* VISIT */
  ...
  DICT.ja = [
    ...
    ['#faq .qa:nth-of-type(6) .q', '<span class="num">07.5</span>アレルギーや食事制限がある場合は？'],
    ['#faq .qa:nth-of-type(6) .a', '鴨が苦手、海鮮アレルギー、卵・乳アレルギー、ベジタリアン、辛いものが苦手——いずれも代わりの一皿があります。ご予約時に必ずお知らせください。記入がない場合、当日の調整はできません。'],
    ['#faq .qa:nth-of-type(7) .q', '<span class="num">07.6</span>食べ終わったら、すぐ帰らないといけませんか？'],
    ['#faq .qa:nth-of-type(7) .a', '急ぐ必要はありません。百十分の火の舞と食事のあと、ここは通常営業に戻ります——そのまま残って、もう一杯どうぞ。'],
    /* VISIT */
  ...
  DICT.ko = [
    ...
    ['#faq .qa:nth-of-type(6) .q', '<span class="num">07.5</span>알레르기나 식이 제한이 있다면요?'],
    ['#faq .qa:nth-of-type(6) .a', '오리를 안 드시는 분, 해산물 알레르기, 달걀·유제품 알레르기, 채식, 매운 것을 못 드시는 분 — 모두 대체 메뉴가 있습니다. 예약할 때 꼭 알려 주세요. 적지 않으면 현장에서는 조정이 어렵습니다.'],
    ['#faq .qa:nth-of-type(7) .q', '<span class="num">07.6</span>식사가 끝나면 바로 가야 하나요?'],
    ['#faq .qa:nth-of-type(7) .a', '서두르지 않아도 됩니다. 백십 분의 불춤과 식사가 끝나면 이곳은 평소 영업으로 돌아갑니다 — 남아서 한 잔 더 하셔도 좋습니다.'],
    /* VISIT */
  ...
  var BOOK_EN = [
    ...
    ['.pg-kicker', 'RESERVE'],
    ['.pg-h1', 'Reserve'],
    ['.pg-lede', 'Pick a date to see <b>live remaining seats</b> — once sent, we hold yours for 72 hours.<br><b>NT$ 2,280</b> per person<small> (+10% service)</small> (was <s>NT$ 2,980</s>) · <b>110 minutes</b>, <b>two fire dances</b>, <b>four courses</b>.'],
    ...
    ['.fld:has(#promo) > label', 'Promo code · if a partner gave you one'],
    ['#promo', 'Code from your partner', 'ph'],
  ...
  var BOOK_JA = [
    ...
    ['.pg-h1', 'ご予約'],
    ['.pg-lede', '日付を選ぶと<b>残席がリアルタイム</b>で表示されます。送信後、72時間お席を確保します。<br><b>NT$ 2,280</b>／1名<small>（別途サービス料10%）</small>（通常 <s>NT$ 2,980</s>）・<b>110分</b>、<b>二幕の火の舞</b>、<b>四皿</b>。'],
    ...
    ['.fld:has(#promo) > label', 'プロモコード · パートナーのコードがあれば'],
    ['#promo', 'パートナーのコード', 'ph'],
  ...
  var BOOK_KO = [
    ...
    ['.pg-h1', '예약'],
    ['.pg-lede', '날짜를 고르면 <b>실시간 잔여석</b>이 표시됩니다. 전송 후 72시간 동안 자리를 잡아둡니다.<br><b>NT$ 2,280</b>/1인<small> (봉사료 10% 별도)</small> (정가 <s>NT$ 2,980</s>) · <b>110분</b>, <b>두 번의 불춤</b>, <b>네 가지 코스</b>.'],
    ...
    ['.fld:has(#promo) > label', '프로모 코드 · 파트너 코드가 있으면'],
    ['#promo', '파트너에게 받은 코드', 'ph'],
  ...
  var LB = {
    zh: { pickFirst: '請先在上方選擇場次', partyPh: '請選擇人數', partyNone: '尚無可選名額', partyN: '{n} 位',
          ...
          bFull: '已滿', bClosed: '報名已截止', bLast: '最後 {n}', bLeft: '剩 {n}', seatWord: '{n} 位',
    ...
    en: { pickFirst: 'Pick a date above first', partyPh: 'How many of you?', partyNone: 'No seats available', partyN: '{n}',
          ...
          bFull: 'Full', bClosed: 'Closed', bLast: 'Last {n}', bLeft: '{n} left', seatWord: '{n}',
    ...
    ja: { pickFirst: 'まず上の日付をお選びください', partyPh: '人数をお選びください', partyNone: '空席がありません', partyN: '{n} 名',
          ...
          bFull: '満席', bClosed: '受付終了', bLast: '残り {n}', bLeft: '残り {n}', seatWord: '{n} 名',
    ...
    ko: { pickFirst: '먼저 위에서 날짜를 선택해 주세요', partyPh: '인원을 선택해 주세요', partyNone: '남은 자리가 없습니다', partyN: '{n}명',
          ...
          bFull: '매진', bClosed: '접수 마감', bLast: '마지막 {n}', bLeft: '{n} 남음', seatWord: '{n}명',
